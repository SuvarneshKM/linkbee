...
export const createPageLink = async (pageLink: NewPageLinkParams) => {
  ...
  const newPageLink = insertPageLinkSchema.parse({
    ...pageLink,
...
export const updatePageLink = async (
  ...
  const newPageLink = updatePageLinkSchema.parse({
    ...pageLink,
    userId: session?.user.id!,
  });
  try {
    const p = await db.pageLink.update({
...
export const deletePageLink = async (id: PageLinkId) => {
  ...
  const { id: pageLinkId } = pageLinkIdSchema.parse({ id });
  try {
    const p = await db.pageLink.delete({
      where: { id: pageLinkId, userId: session?.user.id! },
    });
    return { pageLink: p };
  } catch (err) {
